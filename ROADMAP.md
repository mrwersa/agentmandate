...
| Initiative | Problem and differentiating outcome | Prerequisite and evidence gate | Success measure and non-goal |
|---|---|---|---|
| **Conditional authority** ([private records projected; analysis pending](docs/conditions-delegation.md); medium) | Approval, status, time, and request context can change whether an action is permitted. Add a closed, typed predicate vocabulary with explicit unknown handling. Gates 1–2 narrow the contract and project statement classification and dispatch targets; approval/status/time predicates follow once their operand sources exist. | The contract and private profiles must survive the AWS PostgreSQL and Sentry conditional fixtures before a schema change is proposed. | A reviewer can see which condition opens a path and an absent value never becomes permission. Not arbitrary code or a second general policy language. |
