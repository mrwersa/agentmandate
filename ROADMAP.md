...
| State | Initiatives |
|---|---|
| **Delivered** | Real-graph evidence base, Authority IR, dynamic inventory, conditional authority, delegation analysis, and the Cedar decision/alignment path |
| **Active** | Authority continuity and finite producer cardinality have strict readers, closed IR profiles, and private consumers; the [producer exposure review](docs/bounded-producer-gate-4-review.md) defers a public surface |
| **Ready next** | Producer exposure needs explicit unsupported-composition behavior and CLI failure tests; its synthetic accepted fixture, stable finding codes, and private result envelope are complete. Continuity needs its separate presentation review |
| **Evidence-blocked** | Resource relationships still lack a fixed-binding counterexample; reviewed data flow still lacks a real exfiltration path and annotation study |
...
near-term problem is a bounded evidence and presentation contract: keep
identity continuity separate from state transfer, and distinguish preservation
or conservative translation, fencing and settlement, and an explicit issuer
amendment. General reachability across arbitrary durable agents, memories, and
sessions remains advanced work.
