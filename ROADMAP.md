...
| Initiative | Problem and differentiating outcome | Prerequisite and evidence gate | Success measure and non-goal |
|---|---|---|---|
| **Policy validation and effective diff** ([Cedar managed reader and standalone IR profile implemented; private comparison pending](docs/cedar-effective-diff.md); high) | Syntax-valid per-call policy may still permit an unsafe sequence. Analyze imported policy with tool inventory and compare reachable outcomes across revisions. | Stable IR mappings and executable Cedar/Rego fixtures with known decisions. The Cedar contract separates local-native and managed-enforcement oracles and requires a matched live Deny-to-Allow capture before calling widening delivered. | Equivalent policies produce the same authority summary; a widening policy change yields a counterexample. Not a replacement for native validators. |
