...
| **Resource relationships and provenance** ([evidence audit: gate not met](docs/resource-relationship-audit.md); medium) | Scope counts forget ownership, containment, and whether two bindings denote the same resource. Add a small typed relation vocabulary and preserve binding lineage. | One real false positive or missed path; validate mapping potential against OpenFGA concepts. Existing graphs contain candidate relationships but none changes a reproduced authority result. [Initiative v0.63.4 is the next capture target](https://github.com/mrwersa/agentmandate/issues/104). | The motivating graph becomes more precise without material search regression. Not a complete ReBAC service or arbitrary first-order logic. |
