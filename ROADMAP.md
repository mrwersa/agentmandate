...
| **Active** | Authority-continuity evidence: the [principal-change control](docs/evidence/agentcore-refund-policy/README.md#principal-change-continuity-control) locates tested history at the principal×session boundary, and the [continuation revision matrix](docs/evidence/agentcore-refund-policy/README.md#continuation-revision-matrix) shows a tightened revision still restoring capacity; the deployment cell is now explicitly evidence-blocked by the provider's mandatory Gateway history partition |
| **Ready next** | Capture idempotent retry |
