...
| **Dynamic inventory declarations** (contract proposed; high) | Static scan cannot enumerate provider-built tool lists and must fail closed. Add reviewed inventory boundaries for factories, providers, registries, and deployment configuration. | The [experimental contract](docs/dynamic-inventory.md) must survive AgentKit and Sentry evidence before a public format is chosen. | Drift can prove a declared dynamic boundary complete or explain exactly why it cannot. Never import or execute application code. |
