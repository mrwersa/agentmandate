...
| **Bounded producers and quantities** ([evidence audit: quantity half identified; cardinality gate not met](docs/bounded-producer-evidence-audit.md); medium-low) | `unbounded` versus one binding overstates finite collections and cannot express value relationships such as collateral or 1:1 conversion. Separate cardinality bounds from reviewed quantity relations. | AgentKit supplies collateral, 1:1 conversion, and gross-versus-net quantity distortions. No independent graph yet proves a finite authority-bearing binding count: AWS is multi-output arity, GitHub is conditional production/effect count, and returned bearer IDs are not grants. [Issue #125](https://github.com/mrwersa/agentmandate/issues/125) defines the falsifiable capture gate. | Each feature removes a demonstrated false path while preserving existing breach detection. Not a generic optimization or accounting language. |
