...
| **Import experiments: MCP, A2A, OpenAPI, Cedar, Rego** ([Cedar live widening reproduced; exposure review pending](docs/evidence/agentcore-refund-policy/README.md); medium) | Tool and policy facts live in incompatible formats. Prototype read-only import into the IR while preserving unknowns and unsupported semantics. Cedar starts with a pinned native allow/deny oracle and keeps parsing, validation, decision reproduction, deployment mapping, and authority eligibility separate. | Version-pinned fixtures and a mapping note for each format. Cedar now has matched live one-tool IAM AgentCore revisions and a private fail-closed consumer that reproduces Deny-to-Allow widening; public exposure remains gated. | At least MCP/A2A/OpenAPI inventory and one policy language produce useful, reviewable IR. Not automatic trusted annotation, a Python Cedar evaluator, or production policy compilation. |
...
| Initiative | Problem and differentiating outcome | Prerequisite and evidence gate | Success measure and non-goal |
|---|---|---|---|
| **Policy validation and effective diff** ([Cedar live widening reproduced privately; exposure review pending](docs/cedar-effective-diff.md); high) | Syntax-valid per-call policy may still permit an unsafe sequence. Analyze imported policy with tool inventory and compare reachable outcomes across revisions. | Stable IR mappings and executable Cedar/Rego fixtures with known decisions. The private Cedar consumer aligns exact managed requests and now reproduces a live AgentCore Deny-to-Allow revision; a gate review and versioned output remain required before public exposure. | Equivalent policies produce the same authority summary; a widening policy change yields a counterexample. Not a replacement for native validators. |
