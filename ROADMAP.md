...
| Initiative | Problem and differentiating outcome | Prerequisite and evidence gate | Success measure and non-goal |
|---|---|---|---|
| **Conditional authority** ([private reachability analysis implemented; drift/public gates pending](docs/conditions-delegation.md); medium) | Approval, status, time, and request context can change whether an action is permitted. Add a closed, typed predicate vocabulary with explicit unknown handling. The private path now projects and conservatively consumes statement classification and dispatch targets; approval/status/time predicates follow once their operand sources exist. | The contract, profiles, and trust-failure matrix must survive the AWS PostgreSQL and Sentry conditional fixtures before a schema change is proposed. The complete SELECT-only fixture is synthetic and cannot substitute for deployment evidence. | A reviewer can see which condition narrowed a path and every absent, representative, mixed, expired, or unverifiable value retains the strongest effect. Not arbitrary code or a second general policy language. |
| **Delegation chains and attenuation** ([private records projected; genuine chain still required](docs/conditions-delegation.md); high direction, medium shape) | Caller/service is insufficient when agents act for users or delegate to agents. Track actor, subject, delegator, audience, expiry, and the authority passed at each hop. | The contract's delegation records must survive a genuine delegation capture — an OAuth token exchange, MCP delegated authorization, or A2A grant with a reviewed surface; the committed graphs record a fixed user credential and intersecting principals, neither of which proves a chain. Align terminology with stable standards while isolating drafts. | Detect a delegation that widens rather than narrows authority and produce the shortest chain. Not an identity provider, token issuer, or cryptographic verifier. |
