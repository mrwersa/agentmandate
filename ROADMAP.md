...
| Initiative | Problem and differentiating outcome | Prerequisite and evidence gate | Success measure and non-goal |
|---|---|---|---|
| **Conditional authority** ([contract proposed](docs/conditions-delegation.md); medium) | Approval, status, time, and request context can change whether an action is permitted. Add a closed, typed predicate vocabulary with explicit unknown handling. | The contract must survive the AWS PostgreSQL and Sentry conditional fixtures before a schema change is proposed. | A reviewer can see which condition opens a path and an absent value never becomes permission. Not arbitrary code or a second general policy language. |
| **Delegation chains and attenuation** ([contract proposed](docs/conditions-delegation.md); high direction, medium shape) | Caller/service is insufficient when agents act for users or delegate to agents. Track actor, subject, delegator, audience, expiry, and the authority passed at each hop. | The contract's delegation records must survive the Sentry delegated-token and AWS intersecting-principal fixtures; align terminology with stable standards while isolating drafts. | Detect a delegation that widens rather than narrows authority and produce the shortest chain. Not an identity provider, token issuer, or cryptographic verifier. |
