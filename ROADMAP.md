...
| **Dynamic inventory declarations** (private reconciliation; high) | Static scan cannot enumerate provider-built tool lists and must fail closed. Add reviewed inventory boundaries for factories, providers, registries, and deployment configuration. | The [experimental contract](docs/dynamic-inventory.md), private reader, inventory IR profile, and drift reconciliation preserve complete AgentKit and partial Sentry evidence; public CLI remains gated. | Private drift proves the AgentKit boundary complete and explains why partial, expired, mismatched, or contested evidence cannot. Never import or execute application code. |
