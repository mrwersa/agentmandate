...
| **Import experiments: MCP, A2A, OpenAPI, Cedar, Rego** ([Cedar gates 1–3 implemented; operational candidate selected](docs/cedar-operational-mapping-audit.md); medium) | Tool and policy facts live in incompatible formats. Prototype read-only import into the IR while preserving unknowns and unsupported semantics. Cedar starts with a pinned native allow/deny oracle and keeps parsing, validation, decision reproduction, deployment mapping, and authority eligibility separate. | Version-pinned fixtures and a mapping note for each format. Cedar additionally requires an operational action-to-tool/request mapping before imported decisions constrain reachability. The AgentCore candidate has six finite OpenAPI operations and deployable ENFORCE-mode policies, but native managed-service decisions have not been captured. | At least MCP/A2A/OpenAPI inventory and one policy language produce useful, reviewable IR. Not automatic trusted annotation, a Python Cedar evaluator, or production policy compilation. |
...
selects a pinned AgentCore Gateway sample whose six OpenAPI operations map to
Cedar actions and documented customer/admin controls. Gate 4 remains open on
