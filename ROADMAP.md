...
| **Import experiments: MCP, A2A, OpenAPI, Cedar, Rego** ([Cedar private effective diff implemented; live revision evidence pending](docs/evidence/agentcore-refund-policy/README.md); medium) | Tool and policy facts live in incompatible formats. Prototype read-only import into the IR while preserving unknowns and unsupported semantics. Cedar starts with a pinned native allow/deny oracle and keeps parsing, validation, decision reproduction, deployment mapping, and authority eligibility separate. | Version-pinned fixtures and a mapping note for each format. Cedar now has a live one-tool IAM AgentCore mapping with opposite ENFORCE decisions and a private fail-closed consumer; public exposure and a genuine widening revision remain gated. | At least MCP/A2A/OpenAPI inventory and one policy language produce useful, reviewable IR. Not automatic trusted annotation, a Python Cedar evaluator, or production policy compilation. |
...
values remain representative. The private effective-diff consumer reports
exact-request alignment without mutating reachability; no public command yet
consumes the policy decision.
...
| Initiative | Problem and differentiating outcome | Prerequisite and evidence gate | Success measure and non-goal |
|---|---|---|---|
| **Policy validation and effective diff** ([Cedar private effective diff implemented; live widening pending](docs/cedar-effective-diff.md); high) | Syntax-valid per-call policy may still permit an unsafe sequence. Analyze imported policy with tool inventory and compare reachable outcomes across revisions. | Stable IR mappings and executable Cedar/Rego fixtures with known decisions. The private Cedar consumer now aligns exact managed requests and classifies synthetic policy revisions; a matched live Deny-to-Allow capture remains required before calling widening delivered. | Equivalent policies produce the same authority summary; a widening policy change yields a counterexample. Not a replacement for native validators. |
